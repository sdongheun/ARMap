...
public class GeospatialManager : MonoBehaviour
{
    ...
    public float selectedAnchorCreateDelay = 0.1f; // 같은 건물을 잠시 바라봤을 때만 앵커 생성
    ...
    public float detectionAngle = 90.0f;   // 감지 각도 (도 단위)
    public float verticalAngleLimit = 60.0f;// 수직 각도 제한 (도 단위)
    public float refreshDistance = 100.0f; // 위치 변경에 따른 데이터 새로고침 기준 (미터 단위)
    [Range(0.01f, 0.35f)] public float centerViewportThreshold = 0.32f; // 정보 카드 허용 중심 범위
